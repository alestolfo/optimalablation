...
for ds in dataset_list:
    for i in range(n):
        ...
        # sns.histplot((ablation_data[ds]['oa_specific']['head_losses'] 
        #               / ablation_data[ds][x]['head_losses']).flatten().cpu())
        # plt.title(x)
        # plt.show()
        # print(( .mean().item())
...
#                 positive = ((mlp_nodes[0, mlp_layer] > 0) * 1).item()
#                 true_positives.append(true_positives[-1] + positive)
#                 false_positives.append(false_positives[-1] + (1 - positive))
#             # attn
#             else:
#                 head_layer = node_idx // n_heads
#                 head_idx = node_idx - head_layer * n_heads
#                 positive = ((attn_nodes[0, head_layer, head_idx] > 0) * 1).item()
#                 true_positives.append(true_positives[-1] + positive)
#                 false_positives.append(false_positives[-1] + (1 - positive))
