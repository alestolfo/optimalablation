...
for lamb_path in glob.glob(f"{folder}/*"):
    ...
    if os.path.exists(f"{lamb_path}/mask-status.pkl"):
        with open(f"{lamb_path}/mask-status.pkl", "rb") as f:
    ...
    _, state_dict = retrieve_mask(lamb_path, state_dict=True)
